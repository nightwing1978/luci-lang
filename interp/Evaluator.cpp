		...
		case ast::NodeType::InfixExpression:
			{
				...
				if (   infixExpr->operator_t.type == TokenType::PLUSASSIGN 
					|| infixExpr->operator_t.type == TokenType::MINUSASSIGN 
					|| infixExpr->operator_t.type == TokenType::SLASHASSIGN 
					|| infixExpr->operator_t.type == TokenType::ASTERISKASSIGN ) 
				{
					if (infixExpr->left->type == ast::NodeType::Identifier)
					{
						auto idExpr = static_cast<ast::Identifier*>(infixExpr->left.get());
