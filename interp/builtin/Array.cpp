...
namespace
{
        ...
        size_t nrExpectedArguments)
    {
        if (self.get()->type != obj::ObjectType::Array && self.get()->type != obj::ObjectType::ArrayDouble && self.get()->type != obj::ObjectType::ArrayComplex)
            return std::make_shared<obj::Error>(errorPrefix + ": expected " + toString(obj::ObjectType::Array) + ", got " + toString(self.get()->type));
...
namespace builtin
{
    std::shared_ptr<obj::Object> array_size(const std::shared_ptr<obj::Object> &self, const std::vector<std::shared_ptr<obj::Object>> &arguments)
    {
        ...
        default:
            return std::make_shared<obj::Error>("Method unavailable for type");
        }
        return std::make_shared<obj::Error>("Method unavailable for type");
    }
    ...
    {
        if (self->frozen > 0)
            return std::make_shared<obj::Error>("array clear expects a non-frozen object");
    ...
    {
        if (self->frozen > 0)
            return std::make_shared<obj::Error>("array pop_back expects a non-frozen object");
    ...
    {
        if (self->frozen > 0)
            return std::make_shared<obj::Error>("array push_back expects a non-frozen object");
        ...
        case obj::ObjectType::ArrayDouble:
        {
            ...
            return std::make_shared<obj::Error>("Cannot push a non-double to a [double]");
        }
        case obj::ObjectType::ArrayComplex:
        {
            ...
            return std::make_shared<obj::Error>("Cannot push a non-double to a [complex]");
        }
        }
        return std::make_shared<obj::Error>("Method unavailable for type");
    }
    ...
    std::shared_ptr<obj::Object> array_reserve(const std::shared_ptr<obj::Object> &self, const std::vector<std::shared_ptr<obj::Object>> &arguments)
    {
        ...
        if (arguments[0]->type != obj::ObjectType::Integer)
            return std::make_shared<obj::Error>("reserve: expected argument 1 of type integer");
        auto capacity = static_cast<obj::Integer *>(arguments[0].get());
        if (capacity->value < 0)
            return std::make_shared<obj::Error>("reserve: capacity needs to be zero or larger");
    ...
    {
        if (self->frozen > 0)
            return std::make_shared<obj::Error>("array reverse expects a non-frozen object");
    ...
    {
        if (self->frozen > 0)
            return std::make_shared<obj::Error>("array rotate expects a non-frozen object");

        auto errorObj = validateArguments("rotate", self, arguments, 1);
        if (errorObj)
            return errorObj;
        if (arguments[0]->type != obj::ObjectType::Integer)
            return std::make_shared<obj::Error>("rotate: expected argument 1 of type integer");
